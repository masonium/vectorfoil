...
pub fn barycentric_coords(p: Vec2, tri: &Tri) -> Option<Vec3> {
    let m: TMat3<f32> = TMat3::new(tri.p[0].x, tri.p[1].x, tri.p[2].x,
				   tri.p[0].y, tri.p[1].y, tri.p[2].y,
				   1.0, 1.0, 1.0);
...
impl Tri {
    pub(crate) fn rotated(self, i: usize) -> Tri {
	Tri {
	    p: [self.p[i], self.p[(i+1)%3], self.p[(i+2)%3]],
	    e: [self.e[i], self.e[(i+1)%3], self.e[(i+2)%3]],
	}
    }
}
...
pub fn split_triangle_by_segment(tri: &Tri, p0: Vec2, p1: Vec2) -> Vec<Tri> {
	...
	if is_line_line[i] {
		...
		return split_triangle_aux(tri, i, &isects);
	    }
	}
    }
	...
	(PointTriTest::Outside, PointTriTest::Outside) => {
	    vec![tri.clone()]
	},
	(PointTriTest::Inside(_), PointTriTest::Inside(_)) => {
	    ...
	    split_triangle_aux(tri, edge_isect.0, &isects)
	},
	(PointTriTest::On(e), PointTriTest::Inside(_)) |
	(PointTriTest::On(e), PointTriTest::On(_)) => {
	    split_triangle_aux(tri, e, &isects)
	},
	(PointTriTest::Inside(_), PointTriTest::On(e)) => {
	    split_triangle_aux(tri, e, &isects)
	},
	(PointTriTest::Outside, PointTriTest::On(_)) |
	(PointTriTest::On(_), PointTriTest::Outside) => {
	    vec![tri.clone()]
	},
	_ => {
...
fn split_triangle_aux(tri: &Tri, e: usize, isects: &[RayInt]) -> Vec<Tri> {
    ...
    let p = art_util::easing::lerp(isects[e].t2().unwrap(), tri.p[e], tri.p[(e+1)%3]);
    let e1 = (e+1) % 3;
    let e2 = (e+2) % 3;
	...
	if inside_line_range(t2) {
	    let q = art_util::easing::lerp(t2, tri.p[e1], tri.p[e2]);
	    return vec![ Tri { p: [p, tri.p[e1], q],
			       e: [tri.e[e], tri.e[e1], EdgeType::Split] },
			 Tri { p: [p, q, tri.p[e2]],
			       e: [EdgeType::Split, tri.e[e1], EdgeType::Split] },
			 Tri { p: [p, tri.p[e2], tri.p[e]],
			       e: [EdgeType::Split, tri.e[e2], tri.e[e]] }];
	}
    }
    if let Some(t2) = isects[e2].t2() {
	let q = art_util::easing::lerp(t2, tri.p[e2], tri.p[e]);
	vec![ Tri { p: [p, tri.p[e1], tri.p[e2]],
		    e: [tri.e[e], tri.e[e1], EdgeType::Split] },
	      Tri { p: [p, tri.p[e2], q],
