use crate::common::*;
use crate::primitive::{ZsortPrim, Tri};
...
impl RenderPaths {
	...
	RenderPaths {
	    ...
	    lines: self.lines.into_iter().filter(|rl| rl.edge == EdgeType::Visible).collect()
	}
    }
...
impl<'a> std::iter::FromIterator<&'a ZsortPrim> for RenderPaths {
    ...
    where T: IntoIterator<Item=&'a ZsortPrim>
    {
	...
	for prim in iter {
		...
		Primitive::Line { points } => {
		    rp.lines.push( RenderLine::new(points[0].xy(), points[1].xy(), EdgeType::Visible) );
		}
